...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        //prende tutti i record e li converte in un vettore enumerativo
        //$lte: less than equal , $gte: greater than equal
        collection.find({ "$or": [{ "gender": "f" }, { "weigth": { "$lte": 700 } }] }).toArray(function (err, data) {
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        collection.find({ _id: new _mongodb.ObjectId('61823931b8b9dc10e4699abe') }).toArray(function (err, data) {
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        //prende tutti i record e li converte in un vettore enumerativo 
        //vampires:-1 in decrescente a parità di vampires si ordina in base al nome (name:1)
        collection.find({ "gender": "m" }).project({ "name": 1, "vampires": 1, "_id": 0 }).sort({ "vampires": -1, "name": 1 }).limit(3).toArray(function (err, data) {
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        let collection = db.collection("unicorns");
        //prende tutti i record e li converte in un vettore enumerativo 
        collection.insertOne({ "name": "Apollo", "gender": "m", "loves": ["apple", "energon"] }, function (err, data) {
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        collection.updateOne({ name: 'Arishem' }, { $inc: { vampiress: 1 } }, { "upsert": true }, function (err, data) {
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        collection.find({"gender":"f"},{ "projection": {"_id":0,"name":1, "vampires":1}}).sort({"vampires":-1}).limit(1).toArray(function(err,data){
            if (!err) {
...
mongoClient.connect(CONNECTIONSTRING, function (err, client) {
    if (!err) {
        ...
        collection.replaceOne({"name":"Pluto"},{"name":"Pluto","residenza":"Fossano","loves":["grape","energon"]},function(err,data){
            if (!err) {
